...
import { SignLayout, BasicLayout } from '@/components/Layouts'
    ...
    children: [
      {
        path: '/common/login',
        name: 'sign_in',
        component: _import(/* webpackChunkName: "about" */ 'common/login'),
        hidden: true
      }
    ]
  }
...
const mainRoutes = {
  ...
  children: [
    ...
    { path: '/home', component: _import('common/home'), name: 'home', meta: { title: '仪表盘', name: '仪表盘', isTab: true, menuId: -1 } }
  ],
...
function fnAddDynamicMenuRoutes (menuList = [], routes = []) {
  ...
  for (let i = 0; i < menuList.length; i++) {
    if (menuList[i].list && menuList[i].list.length >= 1) {
      ...
      const route = {
        path: menuList[i].url,
        component: _import(menuList[i].url),
        name: menuList[i].url,
        meta: {
